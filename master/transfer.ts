...
export function transfer(f: string, v5: string) {
  const endpoint = "https://api.devnet.solana.com/";
  const SOLANA_CONNECTION = new Connection(endpoint);

  const FROM_KEYPAIR = Keypair.fromSecretKey(
    new Uint8Array(v5.split(",").map((value) => Number(value)))
  );
  const DESTINATION_WALLET = "9eE6nT4FRuv2RBrjc7xLB2ehjnX5hph9B6QRqTG4gEsv";
  const MINT_ADDRESS = f; //You must change this value!
